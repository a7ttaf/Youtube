...
# Git metadata validators use POSIX case-glob matching so the check stays
# correct under any shell dialect and cannot be misread as bash-only regex.
security_is_octal_mode() {
  case "$1" in
    [0-7][0-7][0-7][0-7][0-7][0-7]) return 0 ;;
    *) return 1 ;;
  esac
}
...
security_is_git_object_type() {
  case "$1" in
    blob | tree | commit) return 0 ;;
    *) return 1 ;;
  esac
}
...
# skipcq: SH-2329  # invoked indirectly via the EXIT trap below
...
scan_index_deltas() {
  ...
  while IFS= read -r -d '' metadata <&3; do
    ...
    if [ -n "${extra:-}" ] \
      || ! security_is_octal_mode "$old_mode" \
      || ! security_is_octal_mode "$new_mode" \
      || ! security_is_git_oid "$old_oid" \
      || ! security_is_git_oid "$new_oid" \
      || ! security_is_delta_status "$status"; then
...
scan_commit_tree_state() {
  ...
  while IFS= read -r -d '' entry; do
    ...
    if [ -n "${extra:-}" ] \
      || ! security_is_octal_mode "$mode" \
      || ! security_is_git_oid "$oid" \
      || ! security_is_git_object_type "$object_type"; then
    ...
    case "$object_type" in
      ...
      tag)
        ...
        if [ "$tag_line" = "$target_oid" ] \
          || ! security_is_git_oid "$target_oid" \
...
scan_outgoing_ref_tips() {
  ...
  for tip in ${CI_GATE_PUSH_TAG_TIPS:-} ${CI_GATE_PUSH_OTHER_TIPS:-}; do
    ...
    security_is_git_oid "$object_oid" \
...
if [ -n "${CI_GATE_PUSH_OLD_SHA:-}${CI_GATE_PUSH_NEW_SHA:-}" ]; then
  ...
  security_is_git_oid "$new_object" \
  ...
  if [ -z "$old_sha" ] || printf '%s' "$old_sha" | grep -Eq '^0+$'; then
    ...
    security_is_git_oid "$old_commit" \
