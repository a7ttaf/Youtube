...
if (NativeRequest) {
  ...
  const nativeRequestAcceptsSignal = (signal: AbortSignal): boolean => {
    try {
      // The instance is the probe: construction succeeding is the only signal
      // that this brand survives native Request's checks.
      const probe = new NativeRequest("http://ums.local/__request-signal-probe__", {
        signal,
      });
      return probe instanceof NativeRequest;
    } catch (error: unknown) {
