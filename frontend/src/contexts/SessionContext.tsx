...
/** Structural slice of the session query result the projection reads. */
type SessionQueryProjection = {
  isFetching: boolean;
  isPending: boolean;
  isError: boolean;
  isSuccess: boolean;
  data: SessionMe | undefined;
  error: unknown;
};

/** Project a settled query result: only an isSuccess payload can authorize a
 * frame; isError clears the principal; anything else stays fail-closed. */
const projectSettledSessionQuery = (
  query: SessionQueryProjection,
): SessionBootstrap => {
  if (query.isError) {
    return {
      status: "error",
  ...
  if (query.isSuccess) {
    return { status: "ready", session: query.data ?? null, error: null };
  }
  return { status: "loading", session: null, error: null };
};

/** Project the boundary-keyed query into a fail-closed bootstrap state:
 * fetching/pending and unrecognized states stay "loading", error clears the
 * principal, and only an isSuccess payload can authorize a frame. */
const projectSessionQuery = (query: SessionQueryProjection): SessionBootstrap => {
  // A refetch of an already-successful query must still mask the prior
  // principal until the boundary's replacement result lands.
  if (query.isFetching || query.isPending) {
    return { status: "loading", session: null, error: null };
  }
  return projectSettledSessionQuery(query);
};

/** A settled success only re-hydrates when it post-dates the current state. */
const successResultIsStale = ({
  requiresHydration,
  status,
  data,
  session,
  ...
  data: SessionMe | undefined;
  session: SessionMe | null;
}): boolean => requiresHydration || status !== "ready" || data !== session;

/** A settled failure only re-fails when it post-dates the current state. */
const failureResultIsStale = ({
  requiresHydration,
  status,
  error,
  sessionError,
  ...
  status: SessionStatus;
  error: unknown;
  sessionError: ApiError | Error | null;
}): boolean =>
  requiresHydration || status !== "error" || sessionError !== error;

type SettledReconcileContext = {
  requiresHydration: boolean;
  status: SessionStatus;
  session: SessionMe | null;
  sessionError: ApiError | Error | null;
...
/** Hydrate the context only when the successful payload post-dates it. */
const reconcileSuccessfulQuery = (
  query: SessionQueryProjection,
  ctx: SettledReconcileContext,
): void => {
  const data = query.data;
  if (data === undefined) return;
  const { requiresHydration, status, session, hydrate, tenantSlug } = ctx;
  if (!successResultIsStale({ requiresHydration, status, data, session })) return;
  hydrate(data, tenantSlug);
};

/** Mirror the query's terminal result into context once per settled boundary
 * outcome; success hydrates the new principal, failure clears it. */
const reconcileSettledSessionQuery = (
  query: SessionQueryProjection,
  ctx: SettledReconcileContext,
): void => {
  if (query.isSuccess) {
    reconcileSuccessfulQuery(query, ctx);
    return;
  }
  if (!query.isError) return;
  const { requiresHydration, status, sessionError, fail, tenantSlug } = ctx;
  if (
    !failureResultIsStale({
      requiresHydration,
      status,
      error: query.error,
      sessionError,
...
export const useSessionBootstrap = (): SessionBootstrap => {
  ...
  useEffect(() => {
    ...
    reconcileSettledSessionQuery(query, {
      requiresHydration,
      status,
  ...
  return projectSessionQuery(query);
};
