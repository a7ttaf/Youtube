...
export type GapExplanationQuery = {
  ...
  // calling the composed endpoint when the current UI session lacks any of the
  // three backend-derived read grants (revenue, finalized payments, bank
  // reconciliation) — the endpoint enforces the union server-side.
  enabled?: boolean;
};
...
//   endpoint gates on the UNION of the revenue, finalized-payment, and
//   bank-reconciliation reads, so a missing grant surfaces as a typed 403.
