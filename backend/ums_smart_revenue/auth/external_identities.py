...
def _require_claim(kind: str, value: str) -> str:
    ...
    # Cc (control) characters such as \x01 are not whitespace but are just as
    # unsafe in a claim; other non-printable categories (Cf format characters)
    # stay accepted because no contract rejects them.
    if any(unicodedata.category(char) == "Cc" for char in value):
        raise InvalidExternalIdentityClaimError(f"{kind} must not contain control characters")
    return value
    ...
    def resolve_user_id(
        ...
        *,
        ...
        try:
            ...
            raise ExternalIdentityStorageError("Unable to load external identity mapping") from exc
