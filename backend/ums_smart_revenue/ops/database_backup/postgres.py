...
def _remove_owned_tool_container(runner: CommandRunner, name: str, *, ownership_token: str) -> None:
    """Remove only the exact short-lived client container created by this process."""
    if name != f"ums-db-client-{ownership_token}" or not re.fullmatch(
        r"[0-9a-f]{32}", ownership_token
    ):
        raise BackupToolError("refusing cleanup for an invalid client identity", exit_code=7)
    listed = runner.text(
        [
            "docker",
            "container",
    ...
    if not identities:
        return
    if len(identities) != 1 or not _SAFE_CONTAINER_RE.fullmatch(identities[0]):
        raise BackupToolError("client cleanup did not resolve exactly once", exit_code=7)
    inspect = _container_inspect(runner, identities[0])
    config = inspect.get("Config")
    labels = config.get("Labels") if isinstance(config, dict) else None
    if not isinstance(labels, dict) or labels.get(_TOOL_LABEL) != ownership_token:
        raise BackupToolError("refusing to remove a foreign client container", exit_code=7)
    immutable_id = inspect.get("Id")
    if not isinstance(immutable_id, str) or not re.fullmatch(r"[0-9a-f]{64}", immutable_id):
        raise BackupToolError("client immutable container id is unavailable", exit_code=7)
    runner.text(
        ["docker", "container", "rm", "--force", "--volumes", immutable_id],
        exit_code=7,
    )
...
# ============================================================================
# Purpose: Execute PostgreSQL client binaries from a new exact-image container,
#   never from the mutable target runtime that receives the restored bytes.
# Database/ORM: Connects only to the selected target network namespace.
# Standards: Absolute binary, read-only/no-capability runtime, cleared libpq
#   overrides, secret-only environment, ownership-bound cleanup on every path.
# Blast Radius: Dump/role/archive confidentiality and exact target selection.
# Connections:
#   - File: backend/ums_smart_revenue/ops/database_backup/restore.py -> replay.
#   - File: docker-compose.yml -> immutable PostgreSQL image source.
# ============================================================================
@contextmanager
def _trusted_client(
    runner: CommandRunner,
    ...
    *,
    ...
    """Yield a docker-start argv for one ownership-bound exact-image client."""
    if program not in {"pg_dump", "pg_isready", "pg_restore", "psql"}:
        raise BackupToolError("unsupported PostgreSQL client program", exit_code=2)
    ownership_token = secrets.token_hex(16)
    name = f"ums-db-client-{ownership_token}"
    environment_args: list[str] = []
    for key in _LIBPQ_OVERRIDE_ENV:
        environment_args.extend(["--env", f"{key}="])
    command_environment: dict[str, str] | None = None
    if needs_database:
        environment_args.extend(["--env", "PGPASSWORD"])
        command_environment = {"PGPASSWORD": source.password}
    network = f"container:{source.container}" if needs_database else "none"
    created = False
    try:
        raw_id = runner.text(
            [
                ...
                *environment_args,
                "--entrypoint",
                f"{_OFFICIAL_CLIENT_BIN}/{program}",
                source.image_id,
                *arguments,
            ],
        ...
        if not re.fullmatch(r"[0-9a-f]{64}", raw_id):
            raise BackupToolError("client creation returned an invalid identity", exit_code=6)
        yield ["docker", "container", "start", "--attach", "--interactive", raw_id]
    except BaseException as primary:
        if created:
            try:
                _remove_owned_tool_container(runner, name, ownership_token=ownership_token)
            except BackupToolError:
                raise BackupToolError(
                    "database client failed and ownership-bound cleanup failed",
                    exit_code=7,
                ) from primary
        raise
    if created:
