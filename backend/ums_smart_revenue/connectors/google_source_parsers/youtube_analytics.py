...
def _analytics_parse_context(request: dict[str, object]) -> _AnalyticsParseContext:
    ...
    # FIX: the runner stamps the post-synthesis ``channel,country`` declaration
    # into query_request.dimensions, so keying strictness on ``{"country"}``
    # never fired in production and the estimatedRevenue metric restriction was
    # dead code. The subset check above already bounds any country query to
    # {channel, country}, so ``country in requested_dimensions`` covers both
    # the wire-level and the post-synthesis declaration shapes.
    strict_country_evidence_query = "country" in requested_dimensions
