...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("watch_time_minutes", sa.Numeric(18, 2), nullable=False, server_default=sa.text("0")),
        sa.Column("confidence_score", sa.Numeric(5, 4), nullable=False, server_default=sa.text("1")),
        ...
        sa.UniqueConstraint("month", "youtube_channel_id", "source_kind", name="uq_monthly_channel_revenue_source"),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint("gross_revenue_usd >= 0", name="ck_monthly_channel_revenue_facts_gross_nonnegative"),
        sa.CheckConstraint(
...
def downgrade() -> None:
    op.drop_index("ix_monthly_channel_revenue_facts_channel_month", table_name="monthly_channel_revenue_facts")
    op.drop_index("ix_monthly_channel_revenue_facts_month", table_name="monthly_channel_revenue_facts")
