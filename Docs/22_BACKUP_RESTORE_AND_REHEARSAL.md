...
| the stack auto-migrated on start, so 38 tables / 180 seeded rows | **exit `8`** — the application tables that held rows are now empty against the directory's watermark (measured: `1 table(s) that held rows at their high-water mark are now empty: org_units`), *and* the cluster's `system_identifier` changed with the volume | None. This is what the per-table watermark added; a whole-directory percentage alone misses it entirely — 180 rows is 98% of a 184-row mark. |
