...
- Router introduction, query/cache architecture changes, and navigation-state
  redesign were excluded from the original P1 slice; they have since landed in
  this PR via `frontend/src/main.tsx`, `frontend/src/router/AppRouter.tsx`,
  and the TanStack Query session layer consumed by `AppShell.tsx`.
...
- Focused domain-403 command: `bunx vitest run`
  ...
  6 files / 68 tests.
- `bunx vitest run --reporter=dot` — passed, 51 files / 627 tests on the merged
  head. The run retained the suite's existing jsdom navigation and React
  `act(...)` stderr warnings; no test failed.
...
- Next: re-run the full Python suite with the disposable PostgreSQL test URL
  and define the separate bounded/streamed artifact-download contract. The
  router, query layer, and design-system work excluded at handoff time has
  since landed on this branch.
