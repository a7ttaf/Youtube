...
- `python scripts/run_validation_gate.py` (with `UMS_TEST_DATABASE_URL` set): **6/6 steps green** (ruff → AST policy → pytest 946 passed → vitest 34 passed → git diff --check worktree → git diff --check staged).
- Pytest delta from PR #41 baseline (821): **+125 tests** (+89 in 17 new test files at submission; +36 review-hardening regressions added during Codex/CodeRabbit review).
