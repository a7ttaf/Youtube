...
The API surface was verified available with the existing stored credential —
`groups.list?onBehalfOfContentOwner=<CO>&mine=true` and
`groupItems.list?groupId=<ID>`, both authorized by `yt-analytics.readonly`.
No new access, no consent, no scope change.
...
**No migration.** `channel_groups.cms_group_id` shipped in PR #159. Single
Alembic head `20260803_0001` verified unchanged.
...
| Single Alembic head (no migration in PR) | `20260803_0001` |
...
Revert the branch. No migration; the route, client, planner, apply module,
and lockdown disappear cleanly. Groups already mirrored keep their synced
names/membership — correct data, no cleanup required.
