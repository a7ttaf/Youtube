...
- Earlier full baseline `uv run pytest -q` — 2,792 passed. The remaining 21
  ...
  `UMS_TEST_DATABASE_URL`; no non-PostgreSQL test failed. The 2,989-pass
  PostgreSQL-backed full suite reported in the PR objective ran on a
  pre-review head; post-merge review fixes (evidence-fence decomposition,
  per-report-type cleanup blocking, strict country-metric enforcement, CSV
  fence alignment) were revalidated on the merged head by the focused
  suites listed below — 218 + 195 + 145 focused tests all passed, plus
  `ruff check`/`ruff format` clean and `git diff --check` clean.
