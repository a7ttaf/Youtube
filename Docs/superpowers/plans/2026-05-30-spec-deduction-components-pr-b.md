...
- **Don't change PR-A (behavioral changes forbidden).** Do not touch `deduction_components.py`, the migration, the ORM, the CLI, or the audit event; do not modify any ingestion/upsert/audit behavior in `deduction_ingestion.py` — except the additive read-only `list_month_components_page` method + `DeductionComponentPage` dataclass added in Task 2. PR-B is purely additive consumption + a read endpoint.
