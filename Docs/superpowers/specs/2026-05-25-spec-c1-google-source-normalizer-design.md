...
- Keep country-dimensional YouTube Analytics rows as persisted evidence, but
  exclude them from canonical fact projection and record each row as an
  explicit `non_projecting_evidence` outcome (`NON_PROJECTING_EVIDENCE` is a
  projection disposition, not a skip reason; only rejected evidence produces
  `INVALID_/DUPLICATE_NON_PROJECTING_EVIDENCE` skips).
...
class NormalizationResult:
    ...
    # U2: accepted/rejected country-evidence decisions land here, NOT in
    # `skipped`. Accepted rows are evidence-only; they never project.
...
Step 5 - Bucket by (channel_id, source_system)
  Before bucketing, _partition_projection_rows classifies every row:
      ...
      a row claiming parser-owned evidence provenance
      (projection_disposition / country-evidence report type / a
      casefolded dimensions key in {country, country_code}):
          classify into a NonProjectingEvidenceOutcome.
          ACCEPTED  -> appended to result.non_projecting_evidence; the row
                       is retained for audit telemetry and NEVER projects.
          REJECTED  -> appended to result.non_projecting_evidence AND to
                       skipped as INVALID_NON_PROJECTING_EVIDENCE or
                       DUPLICATE_NON_PROJECTING_EVIDENCE.
          Evidence rows are NOT silently dropped: every in-scope row
          surfaces in exactly one of created/updated/unchanged, skipped,
          or non_projecting_evidence.
      a valid projecting row (no evidence provenance):
...
`skipped`) before any payload or evidence classification. Malformed Analytics
payloads land in `skipped` and reach the `ROWS_SKIPPED` audit summary.
Country-dimensional Analytics evidence is classified by the evidence fence
instead of silently dropped: accepted rows surface in
`non_projecting_evidence` (audited via the `NON_PROJECTING_EVIDENCE`
lifecycle edge), while rejected or duplicate-provenance rows also land in
`skipped` as `INVALID_NON_PROJECTING_EVIDENCE` or
`DUPLICATE_NON_PROJECTING_EVIDENCE`.
