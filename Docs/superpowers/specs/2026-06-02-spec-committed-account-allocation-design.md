...
2. On the **request-scoped session**, **acquires the finance-month advisory lock + close-row `FOR UPDATE`, held for the whole request transaction** (§7).
3. **Idempotency lookup** (§7) — replays an existing run (200) if the month-scoped key matches the same request.
4. Open-month check → **validate the requested `allocation_method` before compute** (unsupported → 422) → **compute** the allocation **under the held lock** via `compute_month_account_allocation` (server-side — the client never supplies financial results, so inputs cannot change between compute and persist) → **reject-on-unallocated** on the computed result (§8).
5. **Persists** an append-only versioned run (header + lines + unallocated + notes) and **audits** `ALLOCATION_COMMITTED` (summary-only, §9) — all on the **shared request session**. The FastAPI dependency commits the request transaction after the route returns (releasing the advisory lock); the repository neither opens nor commits its own transaction.
6. Returns the committed run (201) — or the replayed run (200).
...
- **`request_fingerprint`** = a stable `blake2b(..., digest_size=16)` hex digest of the **canonicalized request payload** — the client-controlled fields that define *what* is being committed **within the month-scoped key**: **`allocation_method` + `reason`** (the `month` is already the lookup scope, so it is not part of the fingerprint), sorted-key JSON then hashed. It does **not** include the computed result (the key dedups the request, not the data state).
- **Retry semantics** (matched on `(tenant_id, month, idempotency_key)`):
  ...
  - **Existing key (same month), different `request_fingerprint`** — the same `(tenant_id, month, idempotency_key)` reused with a different `allocation_method`/`reason` → **409**. (A different *month* is never a conflict: it is a different lookup key and a new allowed commit.)
...
3. On the **request-scoped session**, **acquire the finance-month advisory lock + load the close row `for_update`, held through steps 4–8** (§7; serializes concurrent same-month / same-key commits).
...
8. Persist run vN + children, then emit the `ALLOCATION_COMMITTED` audit — all on the shared request session (no repository-level commit); the request transaction and its advisory lock commit at the FastAPI dependency boundary after the route returns → **201**.
