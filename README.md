...
| `UMS_TENANT_PRIMARY_CURRENCY` | no | `USD` | ISO-4217 alphabetic code declared as the bootstrap tenant's primary currency in `headers` auth mode (`database` mode ignores it and reads `tenants.primary_currency` instead). A **label only** — UMS performs no currency conversion, so changing it re-labels the declared tenant currency and converts nothing. Unset or blank falls back to `USD`; settings load rejects codes absent from the repository's ISO-4217 snapshot (such as `ZZZ`) even though the database CHECK is shape-only and would accept them. `docker-compose.yml` forwards it from the invoking shell to the `migrate`, `app`, and `app-dev` containers via the `x-app-env` anchor, so the compose stack honours a flip instead of silently booting the default. |
