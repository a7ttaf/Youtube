...
def _module_source(module: ModuleType, project_root: Path) -> str | None:
    """Return the module's canonical project path, or None when unresolvable."""
...
def _enforce_collection_contract(config: pytest.Config, items: Iterable[pytest.Item]) -> None:
    ...
    _enforce_nodeid_ratchet(item_list, lane)


def _enforce_nodeid_ratchet(item_list: tuple[pytest.Item, ...], lane: str) -> None:
    """Re-validate the launcher-ratcheted node IDs inside the execution process."""
    expected_count = os.environ.get("UMS_CI_LANE_ITEM_COUNT")
    expected_digest = os.environ.get("UMS_CI_LANE_NODEID_SHA256")
    if expected_count is None or expected_digest is None:
        return
    nodeids = sorted(item.nodeid.replace("\\", "/") for item in item_list)
    digest = hashlib.sha256("\n".join(nodeids).encode("utf-8")).hexdigest()
    if len(nodeids) != int(expected_count) or digest != expected_digest:
        raise pytest.UsageError(
            f"CI pytest {lane} lane execution collection drifted from the ratcheted manifest: "
            f"count={len(nodeids)} sha256={digest}; expected "
            f"count={expected_count} sha256={expected_digest}"
        )


def _enforce_loaded_support() -> None:
...
def _enforce_fixture_definition(fixturedef: Any) -> None:
...
def pytest_collection_finish(session: pytest.Session):
    ...
    dump_path = os.environ.get("UMS_CI_LANE_NODEID_DUMP")
    if dump_path:
        # Node IDs can embed newlines inside parameter IDs, so stdout line
        # parsing cannot reconstruct the true collected set; the launcher reads
        # this in-process dump for module coverage and the node-ID ratchet.
        nodeids = sorted(item.nodeid.replace("\\", "/") for item in session.items)
...
def pytest_fixture_setup(fixturedef: Any, request: pytest.FixtureRequest):
    ...
    _enforce_fixture_definition(fixturedef)
    yield
