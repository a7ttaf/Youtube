...
def test_revenue_fact_migration_creates_monthly_fact_table():
    ...
    with engine.begin() as connection:
        ...
        columns = {
            column["name"]: column
            for column in inspector.get_columns("monthly_channel_revenue_facts")
        }
        foreign_keys = {
    ...
    assert (
        foreign_keys["fk_monthly_channel_revenue_facts_youtube_channel_id"][
            "referred_table"
        ]
        == "youtube_channels"
    )
    assert foreign_keys["fk_monthly_channel_revenue_facts_youtube_channel_id"][
        "constrained_columns"
    ] == ["youtube_channel_id"]
    assert unique_constraints["uq_monthly_channel_revenue_source"] == (
        "month",
        "youtube_channel_id",
        "source_kind",
    )
...
def _apply_migration(connection) -> None:
    spec = importlib.util.spec_from_file_location(
        "revenue_fact_migration", MIGRATION_PATH
    )
