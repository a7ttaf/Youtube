...
# ============================================================================
def _await_advisory_lock_waiter(engine: sa.Engine, *, lock_key: str) -> None:
    """Wait until a second writer is observably blocked on this lock key.

    Polls pg_locks for an ungranted advisory lock whose classid/objid match the
    high/low 32 bits of ``hashtextextended(lock_key, 0)`` — the exact lock
    ``_next_revision`` takes — rather than any ungranted advisory lock.
    """
    lock_sql = sa.text(
        "SELECT count(*) FROM pg_locks "
        "WHERE locktype = 'advisory' AND NOT granted AND objsubid = 1 "
        "AND classid = (hashtextextended(:lock_key, 0) >> 32)::integer "
        "AND objid = (hashtextextended(:lock_key, 0) & 4294967295)::integer"
    )
    deadline = time.monotonic() + 10
    with engine.connect() as probe:
        while time.monotonic() < deadline:
            if probe.execute(lock_sql, {"lock_key": lock_key}).scalar_one():
                return
            time.sleep(0.05)
    raise AssertionError("second writer never waited on the withholding advisory lock")
...
def _assert_concurrent_rate_revisions(engine: sa.Engine) -> None:
    ...
    release_first = Event()
    lock_key = f"ums:withholding-rate:{_TENANT_A}:{_ACCOUNT_A}:2026-03-01"
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        # Prove the second writer reached pg_advisory_xact_lock before the
        # timeout assertion: an event gap could otherwise let result() time
        # out while the writer is still opening its session.
        _await_advisory_lock_waiter(engine, lock_key=lock_key)
        try:
...
def test_postgres_revision_is_single_head_roundtrips_and_enforces_rls() -> None:
    ...
    assert revision.down_revision == "20260913_0001"
