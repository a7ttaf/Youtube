...
def _write_file(project_root: Path, relative_path: str, source: str) -> None:
    """Return the write file fixture helper."""
...
def _write_test(
    ...
    """Return the write test fixture helper."""
...
def _write_manifest(
    ...
    *,
    ...
    """Return the write manifest fixture helper."""
    rows = [
...
def _basic_project(project_root: Path) -> None:
    """Return the basic project fixture helper."""
...
def test_partition_uses_exact_exhaustive_manifest(tmp_path):
    """Verify partition uses exact exhaustive manifest."""
...
def test_partition_rejects_unassigned_test_module(tmp_path):
    """Verify partition rejects unassigned test module."""
    _basic_project(tmp_path)
...
def test_partition_rejects_stale_manifested_test(tmp_path):
    """Verify partition rejects stale manifested test."""
...
def test_partition_rejects_malformed_manifest(tmp_path, rows, message):
    """Verify partition rejects malformed manifest."""
    _write_test(tmp_path, "tests/unit/test_regular.py")
    _write_test(tmp_path, "tests/db/test_schema.py")
...
def test_partition_requires_support_marker(tmp_path):
    """Verify partition requires support marker."""
    _basic_project(tmp_path)
...
def test_partition_rejects_known_database_access_in_fast_module(tmp_path):
    """Verify partition rejects known database access in fast module."""
...
def test_runtime_guard_rejects_dynamic_database_plugin_in_fast_lane(
    ...
    """Verify runtime guard rejects dynamic database plugin in fast lane."""
    _write_test(
        tmp_path,
        "tests/unit/test_regular.py",
...
def test_runtime_guard_allows_manifested_database_support_in_database_lane(tmp_path):
    """Verify runtime guard allows manifested database support in database lane."""
    _write_test(tmp_path, "tests/unit/test_regular.py")
    _write_test(
        tmp_path,
...
def test_database_capability_fails_closed_and_allows_active_database_item(monkeypatch):
    """Verify database capability fails closed and allows active database item."""
    postgres_env = "UMS_TEST_" + "DATABASE_URL"
...
def test_run_lane_uses_exact_cwd_paths_guard_and_sanitized_environment(tmp_path, monkeypatch):
    """Verify run lane uses exact cwd paths guard and sanitized environment."""
    _basic_project(tmp_path)
    ...
    def fake_run(command, *, cwd, env, check, **kwargs):
        ...
        if "--collect-only" in command:
            ...
            Path(env["UMS_CI_LANE_NODEID_DUMP"]).write_text(
...
def test_collection_gate_uses_exact_lane_argv_and_rejects_disappearing_item(
    ...
    """Verify collection gate uses exact lane argv and rejects disappearing item."""
    _basic_project(tmp_path)
    partition = build_test_partition(tmp_path)
    ...
    def fake_run(command, **kwargs):
        ...
        nodeids = "tests/unit/test_regular.py::test_regular\n" if has_database_argv else ""
        Path(kwargs["env"]["UMS_CI_LANE_NODEID_DUMP"]).write_text(nodeids)
        return SimpleNamespace(
            returncode=0,
            stdout="1 test collected\n",
            stderr="",
...
def test_collection_gate_rejects_changed_lane_node_manifest(tmp_path, monkeypatch):
    """Verify collection gate rejects changed lane node manifest."""
    _basic_project(tmp_path)
    partition = build_test_partition(tmp_path)

    def fake_run(*args, **kwargs):
        """Fake run."""
        Path(kwargs["env"]["UMS_CI_LANE_NODEID_DUMP"]).write_text(
            "tests/unit/test_regular.py::test_regular\n"
        )
        return SimpleNamespace(
            returncode=0,
...
def test_collection_gate_rejects_lane_assignment_swap(tmp_path, monkeypatch):
    """Verify collection gate rejects lane assignment swap."""
    _basic_project(tmp_path)
    partition = build_test_partition(tmp_path)
    ...
    def fake_run(command, **kwargs):
        ...
        Path(kwargs["env"]["UMS_CI_LANE_NODEID_DUMP"]).write_text(selected_nodeid + "\n")
        return SimpleNamespace(
            returncode=0,
            stdout="1 test collected\n",
            stderr="",
...
def test_collection_nodeid_normalizes_module_path_only(tmp_path):
    """Verify collection nodeid normalizes module path only."""
