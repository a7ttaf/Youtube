...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("google", "google-sub\x01123"),
        ("goog\x7fle", "google-sub-123"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("google", "operator @example.com"),
        ("google", "operator@example.com\t"),
        ("google", "operator\x02@example.com"),
    ],
)
...
def test_format_character_claims_remain_accepted(session: Session) -> None:
    """Non-printable claims outside category Cc still query and miss cleanly."""
    repo = SqlAlchemyExternalIdentityRepository(session)
    assert (
        repo.resolve_user_id(
            tenant_id=TENANT_ID,
...
def test_malformed_claims_do_not_query_storage(session: Session) -> None:
    ...
    def guarded_scalars(*_args, **_kwargs):
...
def test_resolve_user_id_translates_storage_error(
    ...
    monkeypatch.setattr(session, "scalars", raise_storage_error)
