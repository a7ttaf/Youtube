...
def principal() -> UserPrincipal:
    return UserPrincipal(
        ...
        role_assignments=[
            RoleAssignment(
                role=RoleKey.CORPORATE_ADMIN, scope=AccessScope.global_scope()
            )
        ],
    )
...
def test_sql_audit_sink_persists_sensitive_audit_record():
    ...
    session.add(
        UserORM(
            id=USER_ID, email="admin@example.com", display_name="Admin", status="active"
        )
    )
    session.commit()
    ...
    record_audit_event(
        ...
        details={
            "old_primary_company_id": "company-old",
            "new_primary_company_id": "company-tv-a",
        },
    )
    session.commit()
