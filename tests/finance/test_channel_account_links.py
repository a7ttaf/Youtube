...
def test_derivation_accepts_projecting_analytics_rows(tmp_path):
    """A projecting youtube_analytics row derives the same link as reporting rows."""
    engine = _engine(tmp_path)
    with Session(engine) as session:
        _source_row(
            session,
            ...
            raw_payload={
                "dimensions": {"channel": "chan-1", "month": "2026-04"},
    ...
    assert created == 1
    assert row.content_owner_id == "owner-1"
    assert row.youtube_channel_id == "chan-1"
    assert row.provenance_source_id.startswith("analytics-proj")
