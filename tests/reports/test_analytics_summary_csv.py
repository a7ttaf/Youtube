...
def test_postgres_projection_fence_compiles_jsonb_key_existence_checks() -> None:
    ...
    # Two key-existence checks remain (country, projection_disposition); the
    # dimensions check is now a strict jsonb_typeof object requirement.
    assert sql.count("jsonb_exists") == 2
