...
def test_register_file_rejects_malformed_actor_user_id():
    with build_session() as session:
        repo = SqlAlchemyRawReportFileRepository(session)
        with pytest.raises(
            RawReportFileValidationError, match="actor_user_id must be a valid UUID"
        ):
            register_defaults(repo, actor_user_id="not-a-uuid")
