...
def test_list_credentials_does_not_surface_cross_tenant_rows():
    with build_session() as session:
        ...
        other_repo = SqlAlchemyConnectorCredentialRepository(session)
        other_repo._tenant_id = OTHER_TENANT_UUID  # noqa: SLF001
        create_default(other_repo, connector_key="adsense", account_id="foreign-1")
        create_default(other_repo, connector_key="adsense", account_id="foreign-2")
        create_default(other_repo, connector_key="adsense", account_id="foreign-3")
        session.commit()
