...
def _worldwide_fixture() -> dict[str, object]:
    """Return a projecting multi-metric worldwide payload for mechanics tests.

    The shared fixture is the production country-evidence shape, where the U2
    contract locks metrics to exactly ``estimatedRevenue``. Tests that need a
    second metric to exercise row-splitting mechanics use this worldwide
    (channel-dimensioned, PROJECTING) variant instead.
    """
    payload = _load_fixture("sample_query_response_2026_04.json")
    headers = [h for h in payload["columnHeaders"] if h["name"] != "country"]
    headers.append({"name": "grossRevenue", "columnType": "METRIC", "dataType": "FLOAT"})
    gross_values = ["1500.000000", "95.000000", "550.000000"]
    rows = [[row[0], row[2], gross_values[index]] for index, row in enumerate(payload["rows"])]
    return {
        **payload,
        "query_request": {
...
def test_reordered_metrics_produce_same_key() -> None:
    ...
    """
    base = _worldwide_fixture()  # metrics: estimatedRevenue,grossRevenue
    reordered = {
        **base,
...
def test_parse_accepts_numeric_metric_values() -> None:
    ...
    """
    base = _worldwide_fixture()
    # columnHeaders order: [channel, estimatedRevenue, grossRevenue].
    # Send the metric cells as JSON numbers (float + int) like the live API.
    numeric = {**base, "rows": [["UC_test_alpha", 1234.56789, 1500]]}
