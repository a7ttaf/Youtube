...
def _audit_record_with_lifecycle(records: list, lifecycle: str):
    """Return the audit record for ``lifecycle`` or fail the test."""
    for record in records:
        if record.details.get("lifecycle") == lifecycle:
            return record
    raise AssertionError(f"expected an audit record with lifecycle={lifecycle!r}")
...
def test_non_projecting_evidence_audit_counts_are_separate_and_private(
    ...
    evidence_record = _audit_record_with_lifecycle(records, "NON_PROJECTING_EVIDENCE")
    ...
    skipped_record = _audit_record_with_lifecycle(records, "ROWS_SKIPPED")
