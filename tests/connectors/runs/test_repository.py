...
def test_find_active_runs_for_scope_returns_only_running_matching_scope(
    ...
    rows = find_active_runs_for_scope(
        session,
        tenant_id=tenant,
        connector_keys=("youtube_reporting",),
        account_id="acct-1",
        report_month="2026-03",
    )
...
def test_find_active_runs_for_scope_matches_hyphen_and_underscore_aliases(
    session: Session,
) -> None:
    """Alias-aware lookup: a RUNNING row opened under a hyphen key is matched
    when the lookup passes the underscore key, and vice versa.

    Pins the fix for: ``POST /connectors/jobs`` previously used an exact
    ``connector_key`` match, so a hyphen-key CLI job (or a previous request
    that stored the row under the public alias) was invisible to an
    underscore-key request for the same logical connector. The
    ``find_active_runs_for_scope`` reader now takes a tuple of alias
    candidates, mirroring the credential + authorise-alias expansion the
    rest of the preflight uses, so a fresh request can't accept a second
    live pull for the same scope just because the two requests used
    different aliases for the same connector.
    """
    from datetime import timedelta
    ...
    tenant = UUID(UMS_TENANT_ID)
    # RUNNING rows under BOTH aliases for the same logical scope.
    hyphen_row = ConnectorRunORM(
        id=uuid4(),
        tenant_id=tenant,
        connector_key="youtube-reporting",
        account_id="acct-1",
    ...
    underscore_row = ConnectorRunORM(
        id=uuid4(),
        tenant_id=tenant,
        connector_key="youtube_reporting",
        account_id="acct-1",
    ...
    # Unrelated RUNNING row in the same tenant; must NOT be returned.
    other = ConnectorRunORM(
        id=uuid4(),
        tenant_id=tenant,
    ...
    session.add_all([hyphen_row, underscore_row, other])
    session.flush()

    # Caller passes the alias-expanded candidate tuple (same expansion
    # ``_credential_key_candidates`` produces) -- the read must match
    # BOTH rows.
    rows = find_active_runs_for_scope(
        session,
        tenant_id=tenant,
        connector_keys=("youtube-reporting", "youtube_reporting"),
        account_id="acct-1",
        report_month="2026-03",
    )
    assert {r.connector_key for r in rows} == {
        "youtube-reporting",
        "youtube_reporting",
    }
    assert all(r.status == "RUNNING" for r in rows)
    # The unrelated scope stays hidden.
    assert {r.connector_key for r in rows}.isdisjoint({"adsense_management"})


def test_find_active_runs_for_scope_rejects_empty_connector_keys(
    session: Session,
) -> None:
    """An empty candidate tuple fails closed with a typed validation error.

    Pins the fix for: passing ``()`` would silently produce a SQL
    ``column IN ()`` (always false) and the route would accept a fresh
    pull even when a live RUNNING row exists for the same scope under
    any alias. The reader must surface that as a 422 instead.
    """
    from ums_smart_revenue.connectors.runs.repository import (
        find_active_runs_for_scope,
    )
