...
def create_bootstrap_app():
    """Create an application with bootstrap channel registry and org access index for testing."""
    app = create_app()
...
def auth_headers(role: str, scope_type: str, scope_id: str | None = None) -> dict[str, str]:
    """Generate authentication headers for a test client given role, scope type, and scope ID."""
    headers = {
        "x-user-id": "user-1",
