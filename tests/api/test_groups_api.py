    ...
    with Session(engine) as session:
        session.add_all(
            [
                ...
                ChannelGroupMemberORM(
                    group_id=GROUP_TV_ID, channel_id=CHANNEL_TV_ROW_ID
                ),
                ChannelGroupMemberORM(
                    group_id=GROUP_MIXED_ID, channel_id=CHANNEL_TV_ROW_ID
                ),
                ChannelGroupMemberORM(
                    group_id=GROUP_MIXED_ID, channel_id=CHANNEL_NEWS_ROW_ID
                ),
                UserORM(
                    id=USER_ID,
        ...
        def flush_with_concurrent_duplicate(*args, **kwargs):
            ...
            if not injected_duplicate:
                ...
                with Session(engine) as other_session:
                    other_session.add(
                        ChannelGroupMemberORM(
                            group_id=GROUP_TV_ID, channel_id=CHANNEL_NEWS_ROW_ID
                        )
                    )
