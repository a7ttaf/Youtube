...
def test_system_integration_user_registers_raw_report_file_metadata_with_audit(
    tmp_path,
):
    ...
    assert (
        response.json()["storage_uri"] == "s3://ums-raw-reports/youtube/2026-03/cms.csv"
    )
...
def test_raw_report_file_registration_rejects_inline_or_local_storage_reference(
    tmp_path,
):
    database_url = build_database_url(tmp_path)
    seed_database(database_url)
