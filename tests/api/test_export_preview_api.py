...
def test_finance_admin_previews_finance_workbook_with_sensitive_audit(tmp_path):
    """Test that finance admins can preview the finance workbook and that the
    audit logs include sensitive events."""
...
def test_scoped_finance_workbook_omits_month_wide_cash_without_attribution(tmp_path):
    """Test that scoped finance workbook previews omit month-wide cash without
    attribution for company scope."""
    database_url = build_database_url(tmp_path)
...
def test_finance_export_preview_includes_revenue_trend_alerts(tmp_path):
    """Test that finance export preview includes revenue trend anomaly alerts when
    previous month data is present."""
    database_url = build_database_url(tmp_path)
...
def test_artifact_metadata_audit_details_marks_incomplete_metadata():
    """Test that artifact metadata audit details correctly identify and mark
...
def test_export_previous_month_rejects_malformed_month():
    """Test that previous month computation rejects malformed month strings by
...
def test_group_scoped_finance_workbook_records_channel_revenue_audit(tmp_path):
    """Test that group-scoped finance workbook preview records channel revenue
    audit events for group scope."""
    database_url = build_database_url(tmp_path)
...
def test_finance_admin_downloads_generated_finance_workbook_with_audit(tmp_path):
    """Verify that finance admins can download generated finance workbooks and that
...
def test_export_operator_cannot_download_finance_workbook(tmp_path):
    """Test that export operator cannot download a finance workbook and receives a
    403 error with no audit events."""
    database_url = build_database_url(tmp_path)
    seed_database(database_url)
    client = TestClient(create_app(database_url=database_url))
...
def test_finance_admin_downloads_generated_executive_pdf_with_audit(
    ...
    """Test that finance admin can download generated executive PDF, triggers audit
    events, and verifies PDF content."""
    artifact_dir = tmp_path / "export-artifacts"
    monkeypatch.setenv("UMS_EXPORT_ARTIFACT_DIR", str(artifact_dir))
    database_url = build_database_url(tmp_path)
...
def test_export_operator_cannot_download_executive_pdf(tmp_path):
    """Test that export operator cannot download an executive PDF and receives a
    403 error with no audit events."""
    database_url = build_database_url(tmp_path)
    seed_database(database_url, export_type="EXECUTIVE_PDF")
    client = TestClient(create_app(database_url=database_url))
