...
def test_finance_admin_records_bank_reconciliation_with_audit(tmp_path):
    """
    Test that a finance admin can record a bank reconciliation entry and that it is audited.
    """
...
def test_bank_reconciliation_record_is_idempotent_for_same_month_reference(tmp_path):
    """
    Test that submitting bank reconciliation for the same month and reference is idempotent.
    """
    database_url = build_database_url(tmp_path)
    seed_database(database_url)
    client = TestClient(create_app(database_url=database_url))
...
def test_finance_viewer_reads_bank_reconciliation_summary_with_audit(tmp_path):
    """
    Test that a finance viewer can read the bank reconciliation summary and audit events.
    """
    database_url = build_database_url(tmp_path)
    seed_database(database_url)
    client = TestClient(create_app(database_url=database_url))
...
def test_finance_month_scoped_admin_records_matching_month(tmp_path):
    """
    Test that a finance-month scoped admin can record bank reconciliation for matching month.
    """
    database_url = build_database_url(tmp_path)
    seed_database(database_url)
    client = TestClient(create_app(database_url=database_url))
...
def test_assistant_cannot_read_bank_reconciliation(tmp_path):
    """
    Test that an assistant analyst cannot read bank reconciliation summary due to missing perm.
    """
    database_url = build_database_url(tmp_path)
    seed_database(database_url)
    client = TestClient(create_app(database_url=database_url))
