...
GROUP_ID = UUID("00000000-0000-0000-0000-00000000c501")
...
def _channel_a_finance_principal() -> UserPrincipal:
    """Return a principal authorized for channel-tv-a revenue reads."""
    return UserPrincipal(
        user_id=str(USER_ID),
        email="net-revenue@example.com",
        direct_permissions=(
...
def _seed_group(database_url: str, *, channel_row_ids: tuple[UUID, ...]) -> None:
    """Seed one active channel group for group-scoped revenue reads."""
    engine = create_engine(database_url)
    with Session(engine) as session:
        session.add(
            ChannelGroupORM(
...
def test_group_scope_reads_only_member_channel_net_revenue_with_audit(tmp_path):
    """A group-scoped net-revenue read is filtered to group members and audited."""
    database_url = build_database_url(tmp_path)
    seed_database(database_url)
    ...
    assert body["calculated_channel_count"] == 1
    assert body["missing_net_source_count"] == 0
    assert body["total_adjusted_gross_revenue_usd"] == "1050"
    assert body["total_net_revenue_usd"] == "930"
    assert [channel["youtube_channel_id"] for channel in body["channels"]] == ["channel-tv-a"]
...
def test_group_scope_requires_revenue_access_to_every_member_channel(tmp_path):
    """A group read is rejected when any member channel is outside the grant."""
    database_url = build_database_url(tmp_path)
    seed_database(database_url)
    _seed_group(database_url, channel_row_ids=(CHANNEL_A_ROW_ID, CHANNEL_B_ROW_ID))
    app = create_app(database_url=database_url)
    app.dependency_overrides[current_principal_from_headers] = _channel_a_finance_principal
    client = TestClient(app)
